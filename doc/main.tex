...
\subsection{values}
\begin{flalign*}
    v ::=\quad \mathtt{null} \mid \mathtt{()} \mid \mathtt{i} \in \mathcal{N} \mid \mathtt{true} \mid \mathtt{false} \mid \mathtt{clos}\left(Q, U\right)
\end{flalign*}
...
\infrule[Add]{
    ...
    \andalso
    \mathit{mtch}\left(I_\mathtt{e1}\right) = \left(\mathtt{r_\mathtt{e1}}, I'\right)
    \andalso\\
    \llceil \mathtt{e2}, \Sigma', \langle I', C', F', \mathit{Um}'\rangle \rrceil = \langle I_\mathtt{e2}, C'', F'', \mathit{Um}''\rangle, \Sigma''
    \andalso
    \mathit{mtch}\left(I_\mathtt{e2}\right) = \left(\mathtt{r_{e2}}, I''\right)
    }{
...
\infrule[Sub]{
    ...
    \llceil \mathtt{e1}, \Sigma, Q\rrceil = \langle I_\mathtt{e1}, C', F', \mathit{Um}'\rangle, \Sigma'
    \andalso
    \mathit{mtch}\left(I_\mathtt{e1}\right) = \left(\mathtt{r_{e1}}, I'\right)
    \andalso\\
    \llceil \mathtt{e2}, \Sigma', \langle I', C', F', \mathit{Um}'\rangle \rrceil = \langle I_\mathtt{e2}, C'', F'', \mathit{Um}''\rangle, \Sigma''
    \andalso
    \mathit{mtch}\left(I_\mathtt{e2}\right) = \left(\mathtt{r_{e2}}, I''\right)
...
\infrule[Not]{
    ...
    \andalso
    \mathit{mtch}\left(I_\mathtt{e}\right) = \left(\mathtt{r_e}, I'\right)
    }{
    \llceil \mathtt{not\ e}, \Sigma, Q\rrceil = 
    \langle I'; \mathtt{Test(r_{e}, 0)}; \mathtt{SetBool(a, 1)}; \mathtt{SetBool(a, 0)}; \mathtt{Return(a)}, C', F', \mathit{Um}'\rangle, \Sigma'
}
...
\infrule[Fun]{
    ...
    \andalso\\
    \Sigma' = \{ \left(\mathtt{x} = \mathtt{-\left(i + 1\right)}\right) \mid \left(\mathtt{x} = \mathtt{i_r}\right)\ \mathrm{as}\ \mathit{xi} \in \Sigma_0 \wedge \mathit{xi}\ \mathrm{is}\ i\mathrm{th\ element\ of}\ \Sigma_0 \}
    \andalso
    \mathit{Um}' = \{ \mathtt{i} \mid \left(\textunderscore = \mathtt{i}\right) \in \Sigma' \}
    \andalso\\
    ...
    \andalso
    \llceil \mathtt{e}, \Sigma_\mathtt{e}', \langle I_\mathtt{e}, C_\mathtt{e}, F_\mathtt{e}, \mathit{Um}' \rangle \rrceil%
    ...
    =
    \langle I; \mathtt{Clos(a, cx, 0)}; \mathtt{Return(a)}, C, F; \mathtt{cx} = \mathit{Clos}, \mathit{Um} \rangle, \Sigma
}
...
\infrule[LetRecFun]{
    ...
    \right\rangle
    \andalso\\
    fv = \left(FV\left(\mathtt{e_1}\right) \textbackslash \{\mathtt{x}\} \cap FV\left(\mathtt{e_2}\right)\right) \textbackslash \{f\}
    \andalso
    \Sigma_0 = \{\left(\mathtt{x=i_r}\right) \mid \left(\mathtt{x=i_r}\right) \in \Sigma \wedge \mathtt{x}\in fv\} \cap \{\mathtt{f = a}\}
    \andalso\\
    \Sigma' = \{ \left(\mathtt{x} = \mathtt{-\left(i + 1\right)}\right) \mid \left(\mathtt{x} = \mathtt{i_r}\right)\ \mathrm{as}\ \mathit{xi} \in \Sigma_0 \wedge \mathit{xi}\ \mathrm{is}\ i\mathrm{th\ element\ of}\ \Sigma_0 \}
    \andalso\\
    \mathit{Um}' = \{i \mid \left(\mathtt{\textunderscore = i}\right) \in \Sigma'\}
    \andalso
    Q_\mathit{init}(\mathtt{e_1}) = \llceil \mathtt{e1}, \Sigma_\mathtt{e1}, \left\langle I_\mathtt{e1}, C_\mathtt{e1}, F_\mathtt{e1}, \textunderscore\right\rangle \rrceil
    \andalso
    \Sigma_\mathtt{e1}' = \Sigma_\mathtt{e1}; \mathtt{x} = \mathtt{0}
    \andalso\\
    ...
    \andalso
    \mathit{Clos}=\left\langle I_\mathtt{e1},', C_\mathtt{e1}', F_\mathtt{e1}', \mathit{Um} @ \mathit{Um'}\right\rangle
    \andalso\\
    I' = I; \mathtt{Clos(a, cx, 1)}
    \andalso
    F' = F; \mathtt{cx}=\mathit{Clos}
    \andalso
    \Sigma' = Sigma; \mathtt{f=a}
    \andalso
    \llceil \mathtt{e2}, \Sigma', \left\langle I', C, F', Um \right\rangle \rrceil = Q', \Sigma''
}{
\llceil \mathtt{let\ rec\ f\ x=e1\ in\ e2}, \Sigma, Q\rrceil = Q', \Sigma''
}

$FV\left(e\right)$ means a set of free variables in $e$
